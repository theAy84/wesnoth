...
function repaint(map_data)
	...
	local villages = map:find(wesnoth.map.filter(f.terrain("*^Vh")))
	set_terrain { "*",
		f.terrain("*^Vh"),
		layer = "overlay",
	}
...
function wild_zones_store(heights)
	for i_height, height in ipairs(heights) do
		for i_temp, temp in ipairs(height) do
			-- oldname: 'regions'
			temp.all_locs = map:find(wesnoth.map.filter(f.terrain(temp.terrain)))
...
function wild_store_cave_zone(map_data)
	map_data.road_in_cave = map:find(wesnoth.map.filter(f.terrain("X*^*")))
end

function wild_store_roads_in_cave_zone(map_data)
	map_data.road_in_cave = map:find(wesnoth.map.filter(f.terrain("R*,Ur")), map_data.road_in_cave)
end
